...
export default class Store extends Component {
	...
	showAvatarCreator = () => {
		this.state.avatar.children[0].children
			.filter((object) => {
				return object.type == 'SkinnedMesh';
			})
			.map((mesh) => {
				this.currentClothing.push({
					name: mesh.name,
	...
	closeAvatarCreator = () => {
		...
		this.currentClothing.map((clothing) => {
			this.state.avatar.getObjectByName(clothing.name).material.map =
				clothing.texture;
			this.state.avatar.getObjectByName(
				clothing.name,
	...
	componentDidMount() {
		...
		this.items.putItems();
		this.collisionDetection.setCollisionObjects(
			this.items.getAllObjectsParts(),
		);
		...
		const animate = () => {
			...
			TWEEN.update();
			let indiDist = this.items.easeOutBounce(
				(clock.elapsedTime % 2) / 2,
			);
			for (let i = 0; i < this.items.items.length; i++) {
				if (
					this.items.items[i].interact &&
					this.items.items[i].indicator.position
				) {
					let initPos = this.items.items[i].position.y + 0.75;
					this.items.items[i].indicator.position.y =
						initPos - indiDist * 0.5;
				}
			}
